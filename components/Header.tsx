...
export const Header: React.FC<HeaderProps> = ({ transparent = false }) => {
  ...
  const menuItems = [
    ...
    { label: 'Nuestro Oficio', path: '/oficio' },
  ...
  return (
    <>
      <nav className={`fixed top-0 z-50 w-full transition-all duration-700 ${transparent && !isMenuOpen
        ...
        <div className="max-w-[1600px] mx-auto px-6 lg:px-12 h-20 md:h-24 flex items-center justify-between">
          ...
          <div className="flex flex-1 justify-end items-center gap-1 md:gap-4 relative z-[70]">
              ...
              <div className="flex items-center gap-2">
                <div className="flex flex-col items-end hidden md:flex">
                  <span className="text-[9px] font-bold uppercase tracking-widest opacity-60">
                    {role === 'admin' ? 'Administrador' : (role === 'user' ? 'Cliente' : 'Identificando...')}
                  </span>
                ...
                {role === 'admin' && (
                  <Link
                    to="/admin"
                    className="p-2.5 hover:text-primary transition-all hover:scale-110"
                    title="Panel Admin"
                  >
                    <Icon name="admin_panel_settings" className="font-light text-2xl" />
                  </Link>
                )}
                <Link
                  to="/historial"
                  ...
                  title="Mis Pedidos"
                >
                  <Icon name="history" className="font-light text-2xl" />
                </Link>
                <button
                  onClick={() => signOut()}
                  className="p-2.5 hover:text-red-500 transition-all hover:scale-110"
                  title="Cerrar Sesión"
                >
                  <Icon name="logout" className="font-light text-2xl" />
                </button>
              </div>
            ) : (
              <Link to="/login" className="p-2.5 hover:text-primary transition-all hover:scale-110">
                <Icon name="person" className="font-light text-2xl" />
              </Link>
            )}
            <Link
              to="/colecciones"
              state={{ showFavorites: true }}
              className="p-2.5 hover:text-primary transition-all hover:scale-110 hidden sm:block"
              title="Mis Favoritos"
            >
            ...
            <button
              onClick={toggleCart}
              className="p-2.5 hover:text-primary transition-all hover:scale-110 relative group"
            >
              <Icon name="shopping_bag" className="font-light text-2xl" />
              {cartCount > 0 && (
                <span className="absolute top-2.5 right-2.5 size-2 bg-primary rounded-full ring-2 ring-white dark:ring-stone-900 group-hover:scale-125 transition-transform flex items-center justify-center text-[8px] text-white">
                </span>
              )}
            </button>
          </div>
      ...
      {/* Menú Móvil Overlay */}
      <div className={`fixed inset-0 z-[60] lg:hidden transition-all duration-700 ease-in-out ${isMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
        }`}>
        ...
        <div className="relative h-full flex flex-col items-center justify-center p-8 space-y-12">
          {menuItems.map((item, index) => (
            <Link
              ...
              className={`text-4xl md:text-5xl font-display-serif font-medium text-text-main dark:text-white hover:text-primary transition-all transform ${isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                }`}
          ...
          {role === 'admin' && (
            <Link
              to="/admin"
              onClick={() => setIsMenuOpen(false)}
              className={`flex items-center gap-3 text-2xl font-display-serif font-medium text-primary hover:text-primary-dark transition-all transform ${isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                }`}
          ...
          {user && (
            <button
              ...
              className={`flex items-center gap-3 text-2xl font-display-serif font-medium text-red-500 hover:text-red-700 transition-all transform ${isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                }`}
              style={{ transitionDelay: `${menuItems.length * 100}ms` }}
            >
          ...
          <div className={`pt-12 border-t border-stone-200 dark:border-white/10 w-full max-w-xs text-center space-y-6 transform transition-all duration-1000 delay-500 ${isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'
            }`}>
            <p className="text-[10px] uppercase tracking-[0.3em] font-black text-stone-400">Hecho a mano en Venezuela</p>
